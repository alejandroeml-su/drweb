            ...
            <div>
              ...
              {tab==='paquete' && (
                <div className="space-y-4">
                  ...
                  {(() => {
                    const misCitas = citas.filter(c=>c.pacienteId===seleccionado.id);
                    const hoy = new Date();
                    const prox = misCitas.filter(c=>{
                      const d=new Date((c.fecha||'')+'T'+(c.hora||'00:00'));
                      return !isNaN(d) && d>=hoy && c.estado!=='cancelada' && c.estado!=='completada';
                    }).sort((a,b)=>((a.fecha||'')+(a.hora||'')).localeCompare((b.fecha||'')+(b.hora||'')));
                    const pasadas = misCitas.filter(c=>!prox.includes(c)).sort((a,b)=>((b.fecha||'')+(b.hora||'')).localeCompare((a.fecha||'')+(a.hora||'')));
                    return (
                      <div className="pt-3 border-t">
                        <h3 className="font-bold text-sm mb-2 flex items-center gap-1" style={{color:C.navy}}>Citas médicas del paciente ({misCitas.length})</h3>
                        {misCitas.length===0 ? (
                          <p className="text-xs text-gray-500 italic">Sin citas registradas. Agéndelas desde el Módulo 1.</p>
                        ) : (
                          <div className="space-y-2">
                            {prox.length>0 && (
                              <div>
                                <div className="text-xs font-bold" style={{color:C.teal}}>Próximas ({prox.length})</div>
                                {prox.slice(0,5).map(c=>(
                                  <div key={c.id} className="text-xs p-2 rounded border mt-1" style={{background:'#EAF7F2',borderColor:C.teal}}>
                                    <strong>{c.fecha}</strong> {c.hora} · {c.tipo||'Consulta'}{c.motivo?` · ${c.motivo}`:''}{c.estado?` · ${c.estado}`:''}
                                  </div>
                                ))}
                              </div>
                            )}
                            {pasadas.length>0 && (
                              <div>
                                <div className="text-xs font-bold text-gray-600">Pasadas ({pasadas.length})</div>
                                {pasadas.slice(0,5).map(c=>(
                                  <div key={c.id} className="text-xs p-2 rounded border mt-1 bg-white">
                                    <strong>{c.fecha}</strong> {c.hora} · {c.tipo||'Consulta'}{c.motivo?` · ${c.motivo}`:''}{c.estado?` · ${c.estado}`:''}
                                  </div>
                                ))}
                              </div>
