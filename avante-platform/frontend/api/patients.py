"""Vercel Serverless Function — Patients CRUD via Supabase REST API."""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if patient_id:
            status, data = _request("GET", f"patients?id=eq.{patient_id}&limit=1", headers_extra={"Accept": "application/json"})
            if isinstance(data, list) and data:
                self._send(200, data[0])
            else:
                self._send(404, {"error": "Not found"})
        else:
            path = f"patients?user_id=eq.{user_id}&order=created_at.desc&limit={limit}"
    ...
    def do_POST(self):
        ...
        status, data = _request("POST", "patients", body, {"Prefer": "return=representation"})
        if isinstance(data, list) and data:
    ...
    def do_PUT(self):
        ...
        status, data = _request("PATCH", f"patients?id=eq.{patient_id}", body, {"Prefer": "return=representation"})
        if isinstance(data, list) and data:
            self._send(200, data[0])
        else:
            self._send(status, data)
    ...
    def do_DELETE(self):
        ...
        status, data = _request("DELETE", f"patients?id=eq.{patient_id}")
        self._send(200, {"message": "Deleted", "id": patient_id})
